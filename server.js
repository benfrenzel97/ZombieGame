...
class GameRoom{
  ...
  _safeTile(pool){return pool[rng(0,pool.length-1)]||{x:this.CX,y:this.CY};}
  _spawnZombies(n){
    ...
    for(const z of this.zombies){
      ...
      if(z.type==='screamer'){
        ...
        if(z.screamTimer<=0){
          ...
          for(const z2 of this.zombies)if(z2.id!==z.id&&Math.hypot(z2.x-z.x,z2.y-z.y)<220)z2.speed=Math.min(z2.speed*1.35,4.0);
