// ─────────────────────────────────────────────────────────────────────────────
//  Zombie Survival — Server v3 (Phase 2)
//  2-slot weapons, ground weapons, reserve ammo, tight economy
// ─────────────────────────────────────────────────────────────────────────────
...
// ─── Weapon Definitions ───────────────────────────────────────────────────────
const WDEFS={
  pistol:  {name:'Pistol',  mag:12,maxMag:12,reserve:36, maxReserve:60, damage:22,spread:0.05,pellets:1,cooldown:8, reload:50,bSpeed:14,bLife:55,color:'#ff9'},
  shotgun: {name:'Shotgun', mag:6, maxMag:6, reserve:12, maxReserve:24, damage:20,spread:0.22,pellets:6,cooldown:32,reload:90,bSpeed:12,bLife:28,color:'#fa4'},
  rifle:   {name:'Rifle',   mag:10,maxMag:10,reserve:30, maxReserve:50, damage:40,spread:0.02,pellets:1,cooldown:5, reload:70,bSpeed:22,bLife:80,color:'#4ff'},
  smg:     {name:'SMG',     mag:20,maxMag:20,reserve:40, maxReserve:80, damage:14,spread:0.10,pellets:1,cooldown:3, reload:65,bSpeed:16,bLife:45,color:'#f4f'},
};

function makeSlot(type){
  const d=WDEFS[type];
  if(!d)return null;
  return{type,name:d.name,mag:d.mag,maxMag:d.maxMag,reserve:d.reserve,maxReserve:d.maxReserve};
}
...
class GameRoom{
  constructor(id){
    ...
    this.pickups=[];        // medkits, wood
    this.groundWeapons=[];  // weapons on ground
    this.barricades=[];
    this.pings=[];
  ...
  _spawnGroundWeapons(n){
    ...
    for(let i=0;i<n;i++){
      ...
      this.groundWeapons.push({
        ...
        type,name:wd.name,
        mag:Math.floor(wd.maxMag*0.5),  // partial mag when found
        maxMag:wd.maxMag,
        reserve:Math.floor(wd.maxReserve*0.3), // scarce reserve
        maxReserve:wd.maxReserve,
      });
    }
  }

  _spawnLoot(){
    const lootWeaponTypes=['shotgun','rifle','smg'];
    for(const room of this.lootRooms){
      // Loot rooms always have a weapon + medkit
      const tx=rng(room.x+1,room.x+room.w-2),ty=rng(room.y+1,room.y+room.h-2);
      const type=lootWeaponTypes[rng(0,lootWeaponTypes.length-1)];
      const wd=WDEFS[type];
      this.groundWeapons.push({
        id:this.ngwid++,
        x:tx*TILE+TILE/2,y:ty*TILE+TILE/2,
        type,name:wd.name,
        mag:wd.maxMag,           // full mag in loot rooms
        maxMag:wd.maxMag,
        reserve:Math.floor(wd.maxReserve*0.6),
        maxReserve:wd.maxReserve,
        loot:true,
      });
      // Also medkit in loot room
      const tx2=rng(room.x+1,room.x+room.w-2),ty2=rng(room.y+1,room.y+room.h-2);
      this.pickups.push({id:this.npid++,x:tx2*TILE+TILE/2,y:ty2*TILE+TILE/2,type:'medkit',amount:40,loot:true});
    }
  }
  ...
  addPlayer(sid,name){
    this.players.set(sid,{
      ...
      slots:[makeSlot('pistol'),null],  // slot 0 = pistol, slot 1 = empty
      activeSlot:0,
      ...
      nearWeaponId:null,  // ID of ground weapon in pickup range
    });
  }
  ...
  handleInput(sid,inp){
    ...
    // Weapon slot swap (Q or scroll)
    if(inp.swapSlot){
      const prev=p.activeSlot;
    ...
    if(inp.reloadReq&&!p.reloading&&p.alive){
      ...
      if(w&&w.mag<w.maxMag&&w.reserve>0){
        p.reloading=true;
        const wd=WDEFS[w.type];
        p.reloadMax=wd.reload;p.reloadTimer=wd.reload;
      }
    }
    // Pick up / swap weapon (F key)
    if(inp.pickupWeapon&&p.alive){
      this._tryPickupWeapon(sid,p);
    }
  }

  _tryPickupWeapon(sid,p){
    // Find nearest ground weapon in range
    ...
    const currentWeapon=p.slots[slot];

    // Drop current weapon at player position (if holding one)
    if(currentWeapon){
      const dropped={
        id:this.ngwid++,
        ...
        maxReserve:currentWeapon.maxReserve,
      };
    ...
    p.slots[slot]={
      type:nearest.type,name:nearest.name,
      mag:nearest.mag,maxMag:nearest.maxMag,
      reserve:nearest.reserve,maxReserve:nearest.maxReserve,
    };
    ...
    io.to(this.id).emit('playerPickedUpWeapon',{pid:sid,slot,weapon:p.slots[slot]});
  }
  ...
  tick(){
    ...
    for(const[sid,p]of this.players){
      ...
      const w=p.slots[p.activeSlot];
      const wd=w?WDEFS[w.type]:null;
      if(p.shooting&&wd&&w&&w.mag>0&&p.shootCooldown<=0&&!p.reloading){
        w.mag--;p.shootCooldown=wd.cooldown;
        ...
        if(w.mag===0&&w.reserve>0){p.reloading=true;p.reloadMax=wd.reload;p.reloadTimer=wd.reload;}
      }

      // Near weapon detection
      p.nearWeaponId=null;
      for(const gw of this.groundWeapons){
        if(Math.hypot(gw.x-p.x,gw.y-p.y)<55){p.nearWeaponId=gw.id;break;}
      }
      ...
      for(const z of this.zombies){
        ...
        if(Math.hypot(b.x-z.x,b.y-z.y)<r){
          ...
          if(z.hp<=0){
            ...
            // Drop ammo for current weapon type on zombie death
            if(Math.random()<0.35){
              // Drop medkit or wood only — ammo comes from ground weapons
              const t=Math.random()<0.5?'medkit':'wood';
              const pk={id:this.npid++,x:z.x,y:z.y,type:t,amount:t==='medkit'?rng(10,20):rng(3,8)};
    ...
    for(const z of this.zombies){
      ...
      if(this.isSolid(Math.floor(probeX/TILE),Math.floor(probeY/TILE))){
        const px=-mdy,py=mdx;
        const c1=this.isSolid(Math.floor((z.x+px*probeD)/TILE),Math.floor((z.y+py*probeD)/TILE));
        const c2=this.isSolid(Math.floor((z.x-px*probeD)/TILE),Math.floor((z.y-py*probeD)/TILE));
      ...
      if(Math.abs(z.x-z.prevX)<0.05&&Math.abs(z.y-z.prevY)<0.05){
        ...
        if(z.stuckTimer>20){
          ...
          for(const[ex,ey]of dirs){
            if(!this.isSolid(Math.floor((z.x+ex*TILE*0.6)/TILE),Math.floor((z.y+ey*TILE*0.6)/TILE))){
